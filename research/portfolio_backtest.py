        ...
        if d in sig_by_date and regime_ok and len(positions) < MAX_POSITIONS:
            ...
            for _, s in cand.iterrows():
                ...
                positions[t] = {"shares": shares, "entry": entry, "stop": s["stop"],
                                ...
                                "weak_rr": s["weak_rr"], "last_close": entry}
                sec_count[s["sector"]] = sec_count.get(s["sector"], 0) + 1

    # --- close anything still open at the last bar ---
    last_d = calendar[-1]
    for t, p in list(positions.items()):
        # Use the real close if the last calendar day has one for this ticker; otherwise
        # (same missing-bar situation as the mark-to-market fix above) fall back to the
        # last known price rather than silently dropping this position from the close-out.
